...
int main()
{
    ...
    while (1)
    {
        ...
        for (int x = 0; x < nScreenWidth; x++)
        {
            ...
            // Figure out how far the ray travels before it hits a wall
            while (!bHitWall && fDistanceToWall < fDepth)
            {
                ...
                if (nTestX < 0 || nTestX >= nMapWidth || nTestY < 0 || nTestY >= nMapHeight) // If out of bounds
                {
                    bHitWall = true;
                    ...
                    if (map[nTestY * nMapWidth + nTestX] == '#')
                    {
                        ...
                        for (int tx = 0; tx < 2; tx++)
                        {
                            for (int ty = 0; ty < 2; ty++)
                            {
                                ...
                                float d = sqrt(vx * vx + vy * vy);  // Size of the vector
                                float dot = (fEyeX * vx / d) + (fEyeY * vy / d);  // Dot product
                                p.push_back(make_pair(d, dot));
                            }
                        }

                        // Sort pairs back front to back
